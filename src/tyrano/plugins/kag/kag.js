tyrano.plugin.kag = {
    ...
    init: function () {
        ...
        var AudioContext =
            window.AudioContext || window.webkitAudioContext || !1;
    ...
    checkUpdate: function (call_back) {
        if ($.isNWJS() || $.isElectron())
            if ("false" != this.kag.config.patch_apply_auto) {
                ...
                patch_path =
                    patch_path + "/" + this.kag.config.projectID + ".tpatch";
    ...
    applyPatch: function (patch_path, flag_reload, call_back) {
        ...
        if (fs.existsSync(patch_path)) {
            ...
            if ("asar" != unzip_path) {
                ...
                new AdmZip(patch_path).extractAllTo(
                    unzip_path + "/update_tmp",
                    !0
                );
                ...
                if ("darwin" == process.platform) {
                    ...
                    let filenames =
                        require("electron").remote.dialog.showOpenDialogSync(
                            null,
                            {
                                properties: ["openFile"],
    ...
    init_game: function () {
        ...
        $(window).bind("load resize orientationchange", function () {
            if (!0 !== flag_resized) {
                ...
                if (
                    ...
                    if (
                        1 == that.kag.stat.is_adding_text ||
                        1 == that.kag.stat.is_wait
                    ) {
                        timerId = setTimeout(function () {
                            $(window).trigger("resize");
                        }, 1e3);
        ...
        if (this.config.keyFocusOutlineWidth) {
            ...
            $.insertRuleToTyranoCSS(
                `:focus.focus { outline-width: ${width}px}`
            );
        }
    ...
    preloadNextVoice: function () {
        ...
        if (next_chara_ptext_pm) {
            next_chara_ptext_pm =
                this.kag.ftag.convertEntity(next_chara_ptext_pm);
            const next_chara_name = next_chara_ptext_pm.name,
                next_chara_voconfig =
                    this.kag.stat.map_vo.vochara[next_chara_name];
            if (next_chara_voconfig) {
                const next_voice_storage = $.replaceAll(
                    ...
                    next_voice_storage_path = $.parseStorage(
                        next_voice_storage,
                        "sound"
                    );
    ...
    html: function (html_file_name, data, callback) {
        ...
        if (this.cache_html[html_file_name]) {
            if (callback) {
                var html = $.templates(this.cache_html[html_file_name]).render(
                    data
                );
                callback($(html));
            }
    ...
    trigger: function (event_name, event_obj = {}) {
        ...
        for (const listener of map[event_name]) {
            ...
            "function" == typeof listener.callback &&
                (callback_return_value = listener.callback.call(
                    this,
                    event_obj
                ));
            if (listener.once) {
        ...
        if (exists_once) {
            const new_listeners = [];
            for (const listener of map[event_name])
                unbind_target_ids.includes(listener.id) ||
    ...
    parseKeyframesForWebAnimationAPI: function (name) {
        if (
            !this.stat.map_keyframe[name] ||
            !this.stat.map_keyframe[name].frames
        )
            return null;
    ...
    changeHowlVolume: function (audio_obj, options = {}) {
        let tag_volume =
            ...
            config_volume =
                void 0 !== audio_obj.__config_volume
                    ? audio_obj.__config_volume
                    : 1;
    ...
    makeFocusable: function (j_elm, tabindex = 0) {
        if ("false" === this.config.useKeyFocus) return;
        ...
        j_elm.on("focusin", (e) => {
            (e.timeStamp - mousedown_timestamp < 10 &&
                mousedown_target === e.target) ||
                ("true" === this.config.keyFocusWithHoverStyle &&
        ...
        this.tmp.screen_effect_timer_id = setTimeout(() => {
            ...
            if ("default" === storage) {
                ...
                def.width &&
                    ...
                    j_effect.css(
                        "font-size",
                        `${(def.width / 15).toFixed(0)}px`
                    );
                def.bgcolor &&
                    j_effect.css("background", $.convertColor(def.bgcolor));
                if (def.color)
                    if ("stop" === _type) {
                        j_effect
                            .children()
    ...
    showLoadingLog(type = "preload") {
        ...
        if (text && "none" !== text) {
            "default" === text &&
                (text =
                    this.kag.getTag("loading_log").default_message_map[type]);
            "notext" === text && (text = "");
